...
void armTriggerAndSleep() {
  ...
  esp_sleep_enable_ext0_wakeup((gpio_num_t)TRIGGER_PIN, 1); // wake on GPIO13 HIGH
...
void setup() {
  ...
  // === Show AP portal only on cold boot or no WiFi config ===
  ...
  if (cause == ESP_SLEEP_WAKEUP_EXT0) {
    Serial.println("Wake from GPIO13 HIGH → capture & upload");
    WiFi.mode(WIFI_STA);
    ...
    esp_camera_deinit();
    sleepCooldown30sOnlyTimer(); // enforce 30s cooldown
  } else {
    Serial.println("Cold boot → arm trigger on GPIO13");
    armTriggerAndSleep();
  }
}
